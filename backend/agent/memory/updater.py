...
from backend.agent.memory.prompt import (
...
def get_memory_data(uid: str | None = None) -> dict[str, Any]:
    ...
    """
    file_path = get_paths().memory_file(uid)
    ...
    if cached is None or cached[1] != current_mtime:
        memory_data = _load_memory_from_file(uid)
        _memory_cache[uid] = (memory_data, current_mtime)
        return memory_data

    return cached[0]


def reload_memory_data(uid: str | None = None) -> dict[str, Any]:
    ...
    _memory_cache[uid] = (memory_data, mtime)
    return memory_data
...
def _load_memory_from_file(uid: str | None = None) -> dict[str, Any]:
    """Load memory data from file.

    Args:
        uid: If provided, loads per-agent memory file. If None, loads global.
...
def _save_memory_to_file(memory_data: dict[str, Any], uid: str | None = None) -> bool:
...
class MemoryUpdater:
    ...
    def _get_model(self):
        ...
        config = get_app_config().models[0]
        model_name = self._model_name or config.model
        return create_chat_model(name=model_name, thinking_enabled=False)

    def update_memory(self, messages: list[Any], uid: str | None = None, ) -> bool:
        ...
        try:
            # Get current memory
            current_memory = get_memory_data(uid)
            ...
            # Apply updates
            updated_memory = self._apply_updates(current_memory, update_data, uid)
            ...
            # Save memory to file
            return _save_memory_to_file(updated_memory, uid)
