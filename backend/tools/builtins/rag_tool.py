...
def retrieve_documents(query: str,file_ids: List[str],runtime: Runtime) -> str:
    ...
    try:
        ...
        embeddings = DashScopeEmbeddings(model=config.embedding_model, dashscope_api_key=config.api_key)
        vectorstore = Chroma(
        ...
        # 3. 知识图谱检索
        graph_results = knowledge_graph.as_retriever(
            search_kwargs={"k": config.top_k_graph}
        ).invoke(query)
